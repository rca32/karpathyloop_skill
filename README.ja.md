...
Codex automation や別の scheduler でループを回したい場合は、`python3 autoresearch/run_autoresearch.py --mode scheduled` を使ってください。scheduled 実行も manual 実行と同じ promotion gate を使うため、勝った candidate が live skill を更新する可能性があります。
...
- automation prompt は次の作業だけを行う形にするのがおすすめです。
  `python3 autoresearch/run_autoresearch.py --mode scheduled` を実行し、`autoresearch/leaderboard.json` と最新の `autoresearch/runs/<RUN_ID>/final.json` を確認し、baseline score、winning candidate、promotion の有無、regressions、人間の review が必要なリスクを inbox に要約します。
