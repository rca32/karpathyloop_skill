...
- Codex automation にそのままつなげられる `--mode scheduled` エントリポイント
...
Codex automation や別の scheduler でループを回したい場合は、`python3 autoresearch/run_autoresearch.py --mode scheduled` を使ってください。scheduled 実行も manual 実行と同じ promotion gate を使うため、勝った candidate が live skill を更新する可能性があります。ホストが macOS か Windows で sleep から直後に戻った場合は、cold network stack を避けるため scheduled 実行をスキップします。

## Codex Automation Support

このリポジトリは、bootstrap と validation の後に使える Codex automation 向けの導線をサポートしますが、automation ファイルを自動でインストールしたり編集したりはしません。

- `python3 autoresearch/run_autoresearch.py --mode scheduled` を公式の Codex automation エントリポイントとして扱ってください。
- 定期実行が本当に必要なときだけ、Codex に automation 作成を依頼してください。
- 推奨する automation 名は `Autoresearch Loop` です。
- automation prompt は次の作業だけを行う形にするのがおすすめです。
  `python3 autoresearch/run_autoresearch.py --mode scheduled` を実行し、`autoresearch/leaderboard.json` と最新の `autoresearch/runs/<RUN_ID>/final.json` を確認し、baseline score、winning candidate、promotion の有無、regressions、人間の review が必要なリスクを inbox に要約します。macOS または Windows の recent wake によってスキップされた場合は、その旨を記録して次の scheduled window で再試行してください。
- automation の作業対象は、対象リポジトリ 1 つだけに固定してください。

v1 では Codex デスクトップ automation を主対象とします。外部 scheduler も利用できますが補助的な経路であり、同じ scheduled エントリポイントを呼ぶ前提です。

git worktree ベースの automation を使うなら、対象リポジトリに少なくとも 1 つの commit が必要です。生成された runner も、一部の automation 環境では既存の `HEAD` が必要になることを note として残します。
